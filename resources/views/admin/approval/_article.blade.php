<div class="card p-5">
    <div class="flex flex-wrap items-center gap-2">
        <span class="tag-blue">Artikel</span>
    ...
    <div class="mt-1 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs font-semibold text-ink-3">
        <span>{{ $item->author }}@if ($item->class) · {{ $item->class }}@endif</span>
        @if ($item->category)
    ...
    <div class="mt-4 flex flex-wrap items-center gap-2">
        <form method="POST" action="{{ route('admin.persetujuan.approveArticle', $item) }}">
            @csrf
        ...
        <button type="button" onclick="document.getElementById('reject-article-{{ $item->id }}').classList.toggle('hidden')" class="btn-outline !border-red-500 !text-red-500 hover:!bg-red-500 hover:!text-cream text-xs">
            <x-icon name="x" class="size-3.5"/>
            Tolak
        </button>
        <a href="{{ route('admin.artikel.edit', $item) }}" class="btn-ghost text-xs">Edit</a>
    </div>
    <div id="reject-article-{{ $item->id }}" class="hidden mt-3">
        <form method="POST" action="{{ route('admin.persetujuan.rejectArticle', $item) }}">
            @csrf
            ...
            <div class="mt-2 flex gap-2">
                ...
                <button type="button" onclick="document.getElementById('reject-article-{{ $item->id }}').classList.add('hidden')" class="btn-ghost text-xs">Batal</button>
            </div>
        </form>
    </div>
</div>
