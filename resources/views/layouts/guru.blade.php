...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>@hasSection('title')@yield('title') — @endif{{ config('app.name', 'Mading Digital') }} · Guru</title>
    ...
    <body class="min-h-screen bg-paper">
        <div class="flex min-h-screen">
            <aside id="guru-sidebar" class="fixed inset-y-0 left-0 z-40 w-64 -translate-x-full border-r-2 border-ink bg-ink text-cream transition-transform duration-300 lg:translate-x-0 lg:static lg:sticky lg:top-0 lg:h-screen">
                <div class="flex h-full flex-col">
                    <div class="flex items-center gap-3 border-b-2 border-cream/15 px-5 py-5">
                        ...
                        <div class="min-w-0">
                            ...
                            <p class="text-[11px] font-semibold uppercase tracking-widest text-cream/50">Panel Guru</p>
                        </div>
                    </div>

                    <nav class="mt-4 flex-1 space-y-1 overflow-y-auto px-3">
                        <a href="{{ route('guru.dashboard') }}" class="flex items-center gap-3 rounded-brutal px-4 py-2.5 text-sm font-bold transition-colors {{ request()->routeIs('guru.dashboard') ? 'bg-acid text-ink' : 'text-cream/70 hover:bg-cream/10 hover:text-cream' }}">
                            <x-icon name="layout" class="size-4.5"/>
                            Dashboard
                        </a>
                        <a href="{{ route('guru.review.index') }}" class="flex items-center gap-3 rounded-brutal px-4 py-2.5 text-sm font-bold transition-colors {{ request()->routeIs('guru.review.*') ? 'bg-acid text-ink' : 'text-cream/70 hover:bg-cream/10 hover:text-cream' }}">
                            <x-icon name="check-circle" class="size-4.5"/>
                            Review Artikel
                            @php $pendingCount = App\Models\Article::pendingReview()->count(); @endphp
                            @if ($pendingCount > 0)
                                <span class="ml-auto rounded-brutal bg-accent px-2 py-0.5 text-[10px] font-bold">{{ $pendingCount }}</span>
                            @endif
                        </a>
                        <a href="{{ route('guru.pengumuman.create') }}" class="flex items-center gap-3 rounded-brutal px-4 py-2.5 text-sm font-bold transition-colors {{ request()->routeIs('guru.pengumuman.*') ? 'bg-acid text-ink' : 'text-cream/70 hover:bg-cream/10 hover:text-cream' }}">
